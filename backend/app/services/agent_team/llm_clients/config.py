...
# Verified against the official Gemini API model list
# (https://ai.google.dev/gemini-api/docs/models) on 2026-07-06 as a current
# stable generateContent id. The live model and the ordered fallback chain
# (POA_LLM_MODEL / POA_LLM_MODEL_CANDIDATES) stay env-configured — do not
# hardcode candidate ids here. Note `gemini-3-flash` is NOT a stable bare id
# on that page (it is `gemini-3-flash-preview`); founder should verify any
# candidate list against the current stable ids before relying on it.
DEFAULT_LIVE_MODEL = "gemini-2.5-flash-lite"
...
class LLMProviderConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.model_candidates) > MAX_MODEL_CANDIDATES:
            raise LLMProviderConfigurationError(
                f"POA_LLM_MODEL_CANDIDATES supports at most {MAX_MODEL_CANDIDATES} models"
            )
        if any(not candidate.strip() for candidate in self.model_candidates):
            raise LLMProviderConfigurationError("POA_LLM_MODEL_CANDIDATES entries must not be empty")
        if len(set(self.model_candidates)) != len(self.model_candidates):
            raise LLMProviderConfigurationError("POA_LLM_MODEL_CANDIDATES entries must be unique")
        if self.model_candidates and self.mode != "live":
            raise LLMProviderConfigurationError("POA_LLM_MODEL_CANDIDATES requires live mode")
        if self.model_candidates and self.model != self.model_candidates[0]:
            raise LLMProviderConfigurationError(
                "model must equal the first POA_LLM_MODEL_CANDIDATES entry when candidates are set"
            )
...
def load_llm_provider_config(env: Mapping[str, str] | None = None) -> LLMProviderConfig:
    ...
    candidates = _model_candidates(values.get("POA_LLM_MODEL_CANDIDATES")) if mode == "live" else ()
    if candidates:
        model = candidates[0]
...
def _model_candidates(value: str | None) -> tuple[str, ...]:
    """Parse the ordered live model chain: trim, drop empties, dedupe in order."""

    if value is None or not value.strip():
        return ()
    ordered = tuple(dict.fromkeys(item.strip() for item in value.split(",") if item.strip()))
    # Cap applies to the deduplicated chain: duplicated ids collapse first, so
    # "m1,m1,m2,m3,m4" is a valid 4-model chain rather than a config error.
