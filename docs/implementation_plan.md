...
Phase goal: implement the first dashboard cockpit on top of existing backend data. This phase is intentionally thin and should not add market quote UI beyond a clear "not available yet" placeholder.

### P11-T1 - React/Vite dashboard shell

- Task id: `P11-T1`
- Title: React/Vite dashboard shell
- Objective: Add a minimal frontend application shell for the portfolio cockpit.
- Files expected to change:
  - `frontend/*`
  - `docs/implementation_plan.md`
- Dependencies: `P10-T8`
- Implementation steps:
  1. Add React/Vite tooling if not already present.
  2. Add shell layout and navigation for account dashboard, positions, freshness, and reports.
  3. Avoid market quote, option screener, TradingAgents, and trade execution UI.
- Acceptance criteria:
  - Frontend shell starts locally and renders a non-marketing dashboard surface.
  - No guaranteed-return or trade-execution language.
- Tests to run:
  - Frontend build/typecheck command chosen during implementation.
- Rollback notes:
  - Revert frontend shell files and plan notes.
- Status: `not_started`

### P11-T2 - user/account selector

- Task id: `P11-T2`
- Title: user/account selector
- Objective: Let the local MVP choose a user and account without public SaaS auth.
- Files expected to change:
  - `frontend/*`
  - `docs/implementation_plan.md`
- Dependencies: `P11-T1`
- Implementation steps:
  1. Fetch users/accounts from existing backend APIs.
  2. Add local selector state.
  3. Show empty/loading/error states.
- Acceptance criteria:
  - A user can select an account for dashboard views.
  - UI does not imply production authentication.
- Tests to run:
  - Frontend build/typecheck and UI smoke test.
- Rollback notes:
  - Revert selector components and tests.
- Status: `not_started`

### P11-T3 - portfolio summary view

- Task id: `P11-T3`
- Title: portfolio summary view
- Objective: Display the existing portfolio summary endpoint in the dashboard shell.
- Files expected to change:
  - `frontend/*`
  - `docs/implementation_plan.md`
- Dependencies: `P11-T2`
- Implementation steps:
  1. Fetch `/accounts/{account_id}/portfolio`.
  2. Show total cash, stock value, option value, internal value, and snapshot timestamps.
  3. Clearly label market quote data as not available yet.
- Acceptance criteria:
  - Dashboard renders backend portfolio values and timestamps.
  - No market quote UI beyond "not available yet".
- Tests to run:
  - Frontend build/typecheck and UI smoke test.
- Rollback notes:
  - Revert summary view changes.
- Status: `not_started`

### P11-T4 - cash/stock/option positions views

- Task id: `P11-T4`
- Title: positions views
- Objective: Show cash, stock, and option positions from internal storage.
- Files expected to change:
  - `frontend/*`
  - `docs/implementation_plan.md`
- Dependencies: `P11-T3`
- Implementation steps:
  1. Add cash positions view.
  2. Add stock positions view.
  3. Add option positions view.
  4. Avoid option chain and screener UI.
- Acceptance criteria:
  - Existing internal positions are visible in the dashboard.
  - No trade execution controls.
- Tests to run:
  - Frontend build/typecheck and UI smoke test.
- Rollback notes:
  - Revert position view components.
- Status: `not_started`

### P11-T5 - broker freshness and warnings panel

- Task id: `P11-T5`
- Title: broker freshness and warnings panel
- Objective: Show broker sync freshness and portfolio warnings prominently.
- Files expected to change:
  - `frontend/*`
  - `docs/implementation_plan.md`
- Dependencies: `P11-T4`
- Implementation steps:
  1. Fetch broker freshness and portfolio warning data.
  2. Display broker freshness separately from future market quote freshness.
  3. Show stale/unknown/error/reauth warnings without over-promising market data.
- Acceptance criteria:
  - UI clearly warns when broker holdings/cash may be stale.
  - UI does not collapse broker sync freshness with market quote freshness.
- Tests to run:
  - Frontend build/typecheck and UI smoke test.
- Rollback notes:
  - Revert freshness/warnings components.
- Status: `not_started`

### P11-T6 - report history placeholder

- Task id: `P11-T6`
- Title: report history placeholder
- Objective: Add the first report history surface backed by Phase 10 APIs.
- Files expected to change:
  - `frontend/*`
  - `docs/implementation_plan.md`
- Dependencies: `P11-T5`
- Implementation steps:
  1. Show report thread list.
  2. Add empty state for no reports.
  3. Link to future report detail workspace without implementing full agent monitor.
- Acceptance criteria:
  - Report history placeholder is visible and wired to backend report APIs.
  - No TradingAgents UI or LLM workflow UI.
- Tests to run:
  - Frontend build/typecheck and UI smoke test.
- Rollback notes:
  - Revert report history placeholder components.
- Status: `not_started`
